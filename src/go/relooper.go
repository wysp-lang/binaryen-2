...
/*
  ...
  *Code Render(RelooperBuilder& Builder, bool InLoop)
  ...
  // Renders the result.
  *Code Render(RelooperBuilder& Builder)
...
func HandleFollowupMultiples(Ret *Code,
                                                Parent *Shape,
                                                 Builder *RelooperBuilder,
                                                 InLoop bool) *Code {
  if !Parent.Next {
...
Branch::Branch(*Code ConditionInit, *Code CodeInit)
...
*Code Block::Render(RelooperBuilder& Builder, bool InLoop) {
  auto* Ret = Builder.makeBlock();
  ...
  *Code Root = nullptr;
  ...
  if (!SwitchCondition) {
    ...
    *Code RemainingConditions = nullptr;
...
*Code SimpleShape::Render(RelooperBuilder& Builder, bool InLoop) {
...
*Code MultipleShape::Render(RelooperBuilder& Builder, bool InLoop) {
  ...
  Ret *Code = Builder.makeBlock(FirstIf);
  Ret = HandleFollowupMultiples(Ret, this, Builder, InLoop);
  if (Next) {
    Ret = Builder.makeSequence(Ret, Next.Render(Builder, InLoop));
  }
...
*Code LoopShape::Render(RelooperBuilder& Builder, bool InLoop) {
  Ret *Code = Builder.makeLoop(Builder.getShapeContinueName(Id),
...
Block* Relooper::AddBlock(*Code CodeInit,
                          *Code SwitchConditionInit) {
...
Branch* Relooper::AddBranch(*Code ConditionInit,
                            *Code CodeInit) {
...
private:
  *Code Canonicalize(*Code Curr) {
  ...
  // to also be nullptr
  template<typename T>
  bool IsPossibleUniquePtrEquivalent(std::unique_ptr<T>& A,
                                            std::unique_ptr<T>& B) {
    if (A == B) {
      return true;
    }
  ...
  bool IsPossibleCodeEquivalent(*Code A,
                                       *Code B) {
    if (A == B) {
      return true;
    }
  ...
  bool IsCodeEquivalent(*Code A, *Code B) {
...
*Code Relooper::Render(RelooperBuilder& Builder) {
  assert(Root);
