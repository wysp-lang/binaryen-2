...
;; Check that string types are emitted properly in the binary format. Also
;; runs --fuzz-exec-before in order to check for interpreter execution.

;; RUN: foreach %s %t wasm-opt --enable-strings --enable-reference-types --enable-gc --roundtrip --fuzz-exec-before -S -o - | filecheck %s
